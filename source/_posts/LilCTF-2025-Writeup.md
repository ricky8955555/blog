...
可以分析出来加密过程中生成的矩阵满足关系 $ C = A^-1 \cdot \mathrm{diag}(\lambda_1, \lambda_2) \cdot A $，所以 $ \lambda_1 $ 和 $ \lambda_2 $ 是矩阵 $C$ 的特征值，在有限域 $ GF(p) $ 上求解特征多项式 $ x^2 - \mathrm{trace}(C)x + \mathrm{det}(C) $ 即可恢复 $ \lambda_1 $ 和 $ \lambda_2 $。

将 $ \lambda_1 $ 和 $ \lambda_2 $ 拼接即可得到 Flag，但顺序不确定。
...
补充上格式得到 Flag 为: `LILCTF{It_w4s_the_be5t_of_times_1t_wa5_the_w0rst_of_t1me5}`
...
可以分析出来矩阵 $C$ 特征值是 $ (a, b, c, d, 0) $，矩阵 $D$ 特征值是 $ (a^k, b^k, c^k, d^k, 0) $，通过求解 $C$ 和 $D$ 的特征值，可建立离散对数方程 $ \lambda_D = \lambda_C^k \mod p $，通过求解离散对数得到密钥 $k$，随后使用 $k$ 作为 AES 密钥即可解密 $\mathrm{msg}$ 得到 Flag。
...
# 计算 C 的特征值和特征向量
eigen = C_mat.eigenmatrix_right()
eigenvalues = eigen[0].diagonal()
eigenvectors = eigen[1].columns()
...
for idx in range(len(eigenvalues)):
    ...
    v = eigenvectors[idx]  # 对应特征向量

    # 验证特征向量并计算 D 的特征值
    non_zero_idx = next(i for i in range(5) if v[i] != 0)
    w = D_mat * v
    mu = w[non_zero_idx] / v[non_zero_idx]
    ...
    # 尝试用 k 解密
    key_bytes = long_to_bytes(int(k_candidate))
